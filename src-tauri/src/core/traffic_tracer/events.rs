...
impl TauriEventBridge {
    ...
    pub fn flush_progress(&self) {
        self.flush_notifications();
        if super::recovery_delivery::global().lock().blocks_progress() {
            return;
        }
        ...
        if let Some(payload) = payload {
            ...
            if !self.emit_event(FrontendWorkerEvent {
                name: EVENT_JOB_PROGRESS,
                payload: payload.clone(),
            }) {
                super::progress_delivery::global()
                    .lock()
                    .dispatch_failed(sequence, payload);
            }
        }
    }
    ...
    fn flush_notifications(&self) {
        ...
        let next = super::notification_delivery::global()
            .lock()
            .take(paused, Instant::now());
        if let Some((sequence, event)) = next {
            ...
            if !self.emit_event(event)
                || !self.emit_event(FrontendWorkerEvent {
                    name: "traffictracer://notification-receipt",
                    payload: sequence.into(),
                })
            {
                super::notification_delivery::global()
                    .lock()
                    .dispatch_failed(sequence, saved);
            }
        }
    }
